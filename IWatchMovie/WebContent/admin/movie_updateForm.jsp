...
// 	String day=sdf.format(mb.getMov_stday());
// 	String[] dat=day.split("-");
// 	int mon=Integer.parseInt(dat[1])-1;
%>
	<script type="text/javascript">
	
		window.onload=function(){
			document.getElementById("genre_code").selectedIndex=<%=mb.getGenre_code()%>;
			document.getElementById("mov_level").selectedIndex=<%=mb.getMov_level()%>;
		}
...
<body>
	...
	<form action="./MovieUpdateAction.ad" method="post" id="fr" name="fr" enctype="multipart/form-data">
	
		<input type="hidden" name="mov_code" value=<%=mb.getMov_code()%>>
		
		<label for="mov_title">영화제목</label>
		<input type="text" id="mov_title" name="mov_title" value="<%=mb.getMov_title()%>"><br />
		
		<label for="mov_dir">영화감독</label>
		<input type="text" id="mov_dir" name="mov_dir" value="<%=mb.getMov_dir()%>"><br />
		
		<label for="mov_act">영화배우</label>
		<input type="text" id="mov_act" name="mov_act" value="<%=mb.getMov_act()%>"><br />
		...
		<textarea cols="50" rows="30" id="mov_info" name="mov_info"><%=mb.getMov_info()%></textarea><br />
		
		<label for="mov_stday">상영개시</label>
		<input type="date" id="mov_stday" name="mov_stday" value="<%=mb.getMov_stday()%>">
		
		<label for="mov_time">상영시간</label>
		<input type="text" id="mov_time" name="mov_time" value="<%=mb.getMov_time()%>"><br />
		...
		<input type="file" id="mov_img" name="mov_img" value="<%=mb.getMov_img() %>"><br />
		<img id="mov_img_pre" style="display:none;"/>
