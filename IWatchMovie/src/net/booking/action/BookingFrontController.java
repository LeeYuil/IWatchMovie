...
public class BookingFrontController extends HttpServlet{
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/DateSchedule.bo"))
		{
			action = new DateSchedule();
			try {
		...
		} else if(command.equals("/TimeSchedule.bo"))
		{
			action = new TimeSchedule();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				// TODO: handle exception
		...
		} else if(command.equals("/SeatList.bo"))
		{
			action = new SeatList();
			try {
				forward = action.execute(request, response);
